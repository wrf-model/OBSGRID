...
!  IMPLICIT NONE
   ...
   CALL read_header ( imax , jmax ) 

   !  Wow, is this how we do the date?  Can't this come from the input file
   !  or the observations?

   PRINT '(A)','Enter date in the format YYYYMMDDHH'
   READ(5,FMT='(A10)') date
   ...
   process_level : DO
      ...
      CALL rdsta (u, v, t, rh, slp, ipress, opress, sid, xc, yc, wqc, tqc, rhqc, pqc, nsta, num, ier, &
      'plotobs_out_' // date(1:4) // '-' // date(5:6) // '-' // date(7:8) // '_' // date(9:10) // ':00:00.0000' )
!     'plotobs_out_sfc_fdda_' // date(1:4) // '-' // date(5:6) // '-' // date(7:8) // '_' // date(9:10) // ':00:00.0000'
...
!        CALL wtstr(xc(i)-m1,yc(i)-m1,lab,isz,0,0)
   ...
   PRINT '(A)','Plot file is called levels.cgm'
