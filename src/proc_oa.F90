...
!  This routine is a driver routine for objective analysis.
   ...
   IF ( oa_3D_type .EQ. 'MQD' .AND. fdda_loop == 1 ) THEN
     ...
     IF ( skip_to_cressman .AND. oa_3D_option == 0 .AND. oa_min_switch ) THEN
       write (*,*)
       write (*,'("###########################################################################")')
       write (*,'("  To few observations have been found to do MQD scheme for all levels      ")')
       write (*,'("  Your options are:                                                        ")')
       write (*,'("    Set oa_3D_option = 1 (if any upper-air level have to few observations, ")')
       ...
       write (*,'("        have to few observations)                                          ")')
